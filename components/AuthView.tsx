...
export const AuthView: React.FC<AuthViewProps> = ({ onAuth, error, isLoading }) => {
  ...
  const toggleMode = () => {
    setIsLogin(!isLogin);
    // Clear specific fields but keep email for convenience
    setPassword('');
    setName('');
  };
    ...
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="glass-panel w-full max-w-md p-8 rounded-2xl border-t border-accent-purple/30 shadow-2xl shadow-purple-900/20 animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!isLogin && (
             ...
             <input
               ...
               className="w-full bg-slate-950/50 border border-slate-800 text-white text-sm rounded-lg focus:ring-accent-purple focus:border-accent-purple block w-full pl-10 p-3 transition-colors"
             />
           </div>
          )}
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-950/50 border border-slate-800 text-white text-sm rounded-lg focus:ring-accent-purple focus:border-accent-purple block w-full pl-10 p-3 pr-10 transition-colors"
            />
            <button
              type="button"
          ...
          <button
            ...
            className="w-full mt-6 py-3 bg-gradient-to-r from-accent-indigo to-accent-purple hover:from-indigo-500 hover:to-purple-500 text-white font-semibold rounded-lg transition-all transform hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-purple-900/20 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <span className="animate-pulse">Authenticating...</span>
            ) : (
              <>
                <span>{isLogin ? 'Enter the Portal' : 'Create Account'}</span>
                <ArrowRight size={16} />
              </>
            )}
          </button>
        ...
        <div className="mt-6 text-center">
          ...
          >
            {isLogin ? "First time here? Begin Initiation" : "Already initiated? Enter Portal"}
          </button>
        </div>
        
        <div className="mt-8 text-[10px] text-center text-slate-600">
          Psychic Star v2.0 // Secure 256-bit connection established.
        </div>
      </div>
    </div>
  );
