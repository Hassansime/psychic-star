...
export const PersonalityQuiz: React.FC<PersonalityQuizProps> = ({ onComplete }) => {
  ...
  const calculateResults = (finalAnswers: Record<number, number>) => {
    ...
    // TIPI Scoring logic (simplified)
    QUESTIONS.forEach(q => {
      let val = finalAnswers[q.id];
      if (q.reverse) val = 8 - val; // Reverse score on 7 point scale
      
      scores[q.trait as keyof typeof scores] += val;
    });

    // Convert to percentage (Max score per trait is 14, min is 2)
    // (Score - 2) / 12 * 100
    ...
    <div className="max-w-2xl mx-auto animate-fade-in p-6">
      <div className="glass-panel rounded-2xl p-8 border-t border-accent-gold/20">
        <div className="flex items-center justify-between mb-8">
          ...
          <span className="font-mono text-xs text-slate-400">{currentStep + 1} / {QUESTIONS.length}</span>
        </div>

        <div className="w-full bg-slate-800 h-1 rounded-full mb-12 overflow-hidden">
          <div className="bg-accent-gold h-full transition-all duration-500" style={{ width: `${progress}%` }}></div>
        </div>
        ...
        <div className="grid grid-cols-7 gap-2 mb-8">
          ...
          {[1, 2, 3, 4, 5, 6, 7].map((num) => (
            <button
              ...
              className="aspect-square rounded-full border border-slate-700 hover:border-accent-gold/50 hover:bg-accent-gold/20 transition-all flex items-center justify-center text-slate-300 hover:text-white focus:ring-2 focus:ring-accent-gold"
            >
              {num}
            </button>
          ))}
